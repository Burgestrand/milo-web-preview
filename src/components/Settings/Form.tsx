...
export default function Form() {
  ...
  return (
    <form className="flex flex-row flex-wrap place-content-center py-2 px-4 gap-2">
      {Object.entries($colors).map(([name, filament]) => (
        <label key={name} className="flex flex-col place-items-center">
          ...
          <select name={name} value={filament.id} onChange={handleColorChange} className="text-xs rounded-md text-black">
